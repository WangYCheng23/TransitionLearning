...
from commit.predict import state_predict
import inspect
import numpy as np
import random
...
class IterDataset(torch.utils.data.IterableDataset):
    ...
    def collect_df(self, csv_files, num_workers=64):
...
class DynamicsModel():
    ...
    def eval(self, test_csv_file_names, test_dfs_map, eval_epoch, batch_size, **kwargs):
        ...
        for s, a, r, next_s, d, rtg, timesteps, mask in test_dataloader:
            ...
            test_predict_next_s = next_state[:,-1,:3]  # 预测的下一时刻最后一步pos
            test_real_next_s = test_next_s[:,-1,:3]  # 真实的下一时刻最后一步的pos
            test_real_current_s = test_s[:,-1,:3]   # 真实的当前时刻最后一步的pos
            
            predict_next_end_effector_coornidates = excavator_arm_fk(test_predict_next_s)
            real_next_end_effector_coornidates = excavator_arm_fk(test_real_next_s)
            real_current_end_effector_coornidates = excavator_arm_fk(test_real_current_s)
            
            end_distance = torch.norm((predict_next_end_effector_coornidates-real_next_end_effector_coornidates), p=2, dim=-1)
            real_end_distance = torch.norm((real_next_end_effector_coornidates-real_current_end_effector_coornidates), p=2, dim=-1)
            end_distance_ratio = torch.div(end_distance,(real_end_distance+1e-3))
            
            mean_end_distance = torch.mean(end_distance)
            mean_end_distance_ratio = torch.mean(end_distance_ratio)
            
            score_end_distance = 100 - 100*(mean_end_distance/1000)  #末端误差评分
            score_end_distance_ratio = 100 - 100*(mean_end_distance_ratio/1000)  #末端误差比例评分	
            ...
            print(f"末端误差: {score_end_distance} | 末端误差比例: {score_end_distance_ratio}")
