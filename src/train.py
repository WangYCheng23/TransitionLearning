...
class IterDataset(torch.utils.data.IterableDataset):
    def __init__(self, env, mode, csv_file_names, dfs_map, **kwargs):
        ...
        self.data_iter = self.get_data()    #获取数据实现
        print(f"[{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}]"
...
class DynamicsModel():
    ...
    def joint_dis_loss(self, real_current_s, real_next_s, prediction_s, attention_mask):
        ...
        # 前向运动学转换
        # predict_next_end_effector_coornidates = excavator_arm_fk(predict_next_state[:,0:3])
        # real_next_end_effector_coornidates = excavator_arm_fk(real_next_state[:,0:3])
        ...
        #         f"score_end_distance:{score_end_distance},score_end_distance_ratio:{score_end_distance_ratio}")
        #########################score#########################
    ...
    def save_accelerate_model(self, force_save=False):
        ...
        # run_name = f"{date_string}_{time_string}"
        run_name = 'default'
